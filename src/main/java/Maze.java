...
//        this.setLocationRelativeTo(null);
        ...
        new Thread(() -> {
            ...
            switch (this.algorithm) {
                case Definitions.ALGORITHM_DFS:
                    ...
                    Square startSquare=new Square(this.startRow,startColumn);
                    stack.push(startSquare);
                    while (!stack.empty())
                    {
                        Square currentSquare=stack.pop();
                        if(currentSquare.getX()==(this.values.length-1) && currentSquare.getY()==(this.values.length-1) )
                        {
                            ...
                            ArrayList<Square>suns=makeSuns(currentSquare);
                            for (Square square:suns)
                            {
                                if(!visited[square.getX()][square.getY()]&&this.values[square.getX()][square.getY()]==Definitions.EMPTY){
                                    stack.push(square);
                                }
    ...
    public ArrayList<Square> makeSuns(Square square) {
        ArrayList<Square> suns=new ArrayList<>();
        if(square.getX()-1>=0)
        {
            suns.add(new Square(square.getX() - 1, square.getY()));
        }
        if(square.getX()+1<this.values.length)
        {
            suns.add(new Square(square.getX() + 1, square.getY()));
        }
        if(square.getY()-1>=0)
        {
            suns.add(new Square(square.getX(), square.getY() - 1));
        }
        if(square.getY()+1<this.values.length)
        {
            suns.add(new Square(square.getX(), square.getY() + 1));
        }
        return suns;
